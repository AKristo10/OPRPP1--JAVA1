 ...
 * @author Luka Mesaric
 */
public class CalculatorMesaric extends JFrame {
	...
	private final CalcImpl model = new CalcImpl();
	...
	public static void main(String[] args) {
		SwingUtilities.invokeLater(() -> {
			new CalculatorMesaric().setVisible(true);
		});
	}

	/** Default constructor. */
	public CalculatorMesaric() {
	...
	private void initGUI() {
		...
		setUpSimpleButton(".", "5,5", new ActionListener() {
	 ...
	 */
	private JButton setUpSimpleButton(String text, String position, ActionListener listener) {
	 ...
	 * @return constructed button
	 */
	private BinaryOperationButton prepareBinaryOperationButton(String text, DoubleBinaryOperator operation,
	...
	private void setUpInvertibleOps(JCheckBox invertCb) {
		...
		prepareInvertibleButton(
				new InvertibleOperationButton<DoubleUnaryOperator>("sin", "arcsin", Math::sin, Math::asin),
				invOpListener, invertCb, "2,2");
		prepareInvertibleButton(
				new InvertibleOperationButton<DoubleUnaryOperator>("cos", "arccos", Math::cos, Math::acos),
				invOpListener, invertCb, "3,2");
		prepareInvertibleButton(
				new InvertibleOperationButton<DoubleUnaryOperator>("tan", "arctan", Math::tan, Math::atan),
				invOpListener, invertCb, "4,2");
		prepareInvertibleButton(new InvertibleOperationButton<DoubleUnaryOperator>("ctg", "arcctg",
				x -> 1 / Math.tan(x), x -> Math.atan(1 / x)), invOpListener, invertCb, "5,2");
		prepareInvertibleButton(
				new InvertibleOperationButton<DoubleUnaryOperator>("log", "10^x", Math::log10, x -> Math.pow(10, x)),
				invOpListener, invertCb, "3,1");
		prepareInvertibleButton(
				new InvertibleOperationButton<DoubleUnaryOperator>("ln", "e^x", Math::log, x -> Math.pow(Math.E, x)),
				invOpListener, invertCb, "4,1");
		prepareInvertibleButton(new InvertibleOperationButton<DoubleBinaryOperator>("x^n", "x^(1/n)", Math::pow,
				(x, n) -> Math.pow(x, 1.0 / n)), constructBinaryOpsListener(), invertCb, "5,1");
	 ...
	 * @param position button position
	 */
	private void prepareInvertibleButton(InvertibleButton button, ActionListener listener, JCheckBox trigger,
			String position) {
	 ...
	 * @author Luka Mesaric
	 */
	private static class BinaryOperationButton extends JButton implements Supplier<DoubleBinaryOperator> {
		/** Serial version UID. */
	...
	private static class InvertibleButton extends JButton {
		...
		private InvertibleButton(String textRegular, String textInverted) {
			...
			if (d1 == null || d2 == null) {
				...
				prefSize = new Dimension(Math.max(d1.width, d2.width), Math.max(d1.height, d2.height));
			}
		}
	 ...
	 * @author Luka Mesaric
	 */
	private static class InvertibleOperationButton<E> extends InvertibleButton implements Supplier<E> {
		/** Serial version UID. */
		 ...
		 *                          in inverted state
		 */
		private InvertibleOperationButton(String textRegular, String textInverted, E operationRegular,
				E operationInverted) {
